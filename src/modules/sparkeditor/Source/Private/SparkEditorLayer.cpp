...
namespace Spark
{
	...
	SparkEditorLayer::SparkEditorLayer()
	{
		...
		m_ImGuiFramebuffer = IFramebuffer::Create(props);
	}
	...
	void SparkEditorLayer::OnAttach()
	{
		...
		PerspectiveCameraParameters params(45.0f, 16.0f / 9.0f, 0.1f, -1000.0f);
		...
		auto texshader = m_ShaderLibrary->Load("./Assets/Shaders/TextureMesh.hlsl");
	...
	void SparkEditorLayer::OnUpdate(Spark::Timestep delta)
	{
		...
		//        Renderer2D::BeginScene(*m_CameraActor->GetComponent<CameraComponent>()->GetCamera());

		//        GWorld->GetContext().Each<CameraComponent>([&](EntityID e, CameraComponent* meshComponent){
		//            auto transform = GWorld->GetContext().Get<TransformComponent>(e);
		//            Renderer2D::DrawQuad(transform->Position, {1,1},
		//            glm::vec4(transform->Position.x,transform->Position.y,transform->Position.z,1));
		////            m_Texture->Bind();
		...
		GWorld->GetContext().Each<MeshComponent>([&](EntityID e, MeshComponent *meshComponent) {
			auto transform = GWorld->GetContext().Get<TransformComponent>(e);
			transform->Scale = glm::vec3(.01f, .01f, .01f);
			//                    m_Texture->Bind();
			//                    Renderer2D::DrawQuad(transform->Position, {10,10}, glm::vec4(1,0,0,1));
			for (auto va : meshComponent->GetVertexArrays())
				Renderer::Submit(m_ShaderLibrary->Get("TextureMesh"), va, transform->GetMatrix());
			//                    m_Texture->Unbind();
		});

		//        Renderer2D::DrawQuad(glm::vec3({0.0f, 0.0f, 0.0f}), glm::vec2({1.0f, 1.0f}),
		//        glm::vec4({1.0f, 1.0f, 1.0f, 1.0f}), m_Texture); Renderer2D::DrawQuad(glm::vec3({0.1f, 0.0f, -0.1f}),
	...
	void SparkEditorLayer::OnEvent(Event &event)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(
			std::bind(&SparkEditorLayer::HandleKeyInputEvents, this, std::placeholders::_1));
