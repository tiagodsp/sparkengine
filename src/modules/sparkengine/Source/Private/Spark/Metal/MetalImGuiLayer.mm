...
namespace Spark
{
	...
	void MetalImGuiLayer::End()
	{
		...
		// Create command buffer and command encoder for rendering

		metal_context->m_Surface = [metal_context->m_Layer nextDrawable];
		metal_context->m_DefaultPass.colorAttachments[0].texture = metal_context->m_Surface.texture;
		metal_context->m_CommandBuffer = [metal_context->m_CommandQueue commandBuffer];
		...
		[metal_context->m_CommandBuffer presentDrawable:metal_context->m_Surface];
		[metal_context->m_CommandBuffer commit];
	...
	void MetalImGuiLayer::OnEvent(Event &event)
	{
		...
		dispatcher.Dispatch<MouseScrollEvent>(
			std::bind(&MetalImGuiLayer::OnMouseScrollEvent, this, std::placeholders::_1));
		dispatcher.Dispatch<MouseMovedEvent>(
			std::bind(&MetalImGuiLayer::OnMouseMovedEvent, this, std::placeholders::_1));
		dispatcher.Dispatch<WindowResizeEvent>(
			std::bind(&MetalImGuiLayer::OnWindowResizedEvent, this, std::placeholders::_1));
		dispatcher.Dispatch<KeyPressedEvent>(
			std::bind(&MetalImGuiLayer::OnKeyPressedEvent, this, std::placeholders::_1));
		dispatcher.Dispatch<KeyReleasedEvent>(
			std::bind(&MetalImGuiLayer::OnKeyReleasedEvent, this, std::placeholders::_1));
